...
# ── Données SPARC ──
sparc_names = ['NGC 2841', 'NGC 7331', 'Voie Lactée', 'NGC 6946', 'M33', 
               'NGC 3198', 'NGC 2403', 'NGC 6503', 'IC 2574', 'DDO 154', 'NGC 3741']
sparc_masses = np.array([1.1e11, 9.0e10, 6.0e10, 5.5e10, 5.0e9, 
                         3.0e10, 8.0e9, 5.0e9, 1.8e9, 4.0e8, 2.0e8])
sparc_vobs = np.array([302, 250, 220, 200, 130, 150, 135, 116, 66, 47, 50])

# ── Figure ──
fig = plt.figure(figsize=(16, 10), facecolor='#0a0a1a')
fig.canvas.manager.set_window_title("Frame Theory: Complete Simulation & Validation")
fig.suptitle("Frame Theory: Comparison with Newton & Empirical Validation", color='white', fontsize=18, fontweight='bold', y=0.96)

# GridSpec for 4 panels
gs = GridSpec(2, 2, width_ratios=[1, 1.2], height_ratios=[1, 1.2])

ax_O = fig.add_subplot(gs[0, 0], facecolor='#0d0d2b')
ax_P = fig.add_subplot(gs[0, 1], facecolor='#0d0d2b')
ax_G = fig.add_subplot(gs[1, 0], facecolor='#0d0d2b')
ax_S = fig.add_subplot(gs[1, 1], facecolor='#0d0d2b')
plt.subplots_adjust(bottom=0.28, hspace=0.35, wspace=0.25)

for ax in [ax_O, ax_P, ax_G, ax_S]:
...
ax_O.set_title("Orbits", color='white', fontsize=13)
...
# 2. Photons
ax_P.set_xlim(-80, 80)
# L'axe Y est maintenant dynamique, initialisé ici temporairement
ax_P.set_ylim(42, 52) 
ax_P.set_title("Photon Deflection (Zoom ×2 Eddington)", color='white', fontsize=13)
ax_P.axvline(0, color='yellow', lw=1, ls='--', alpha=0.3)
pt_mass_P, = ax_P.plot([0], [42], 'o', color='yellow', ms=70, alpha=0.9, zorder=1) # Masse rendue dynamique
...
ax_P.text(-76, 50.8, r"$\theta_{Frame} = \frac{4GM}{bc^2}$", color='#ff8844', fontsize=14, bbox=eq_box)
...
ln_Rt  = ax_G.axvline(0, color='white', ls='--', alpha=0.6, label=r'Radius $R_t$')
...
ax_G.text(1.0, 40, r"$v^2_{Frame}(r) = \frac{GM}{r} + \frac{GM}{R_t}$", color='#ff8844', fontsize=14, bbox=eq_box)

# 4. SPARC Validation
ax_S.set_title("Empirical Validation (SPARC Catalog)", color='white', fontsize=13)
ax_S.set_xlabel(r"Baryonic Mass $M_{bar} (M_\odot)$")
ax_S.set_ylabel(r"Asymptotic Velocity $v$ (km/s)")
ax_S.set_xscale('log')
ax_S.set_yscale('log')
ax_S.grid(True, which="both", ls="--", alpha=0.2, color='white')

ax_S.scatter(sparc_masses, sparc_vobs, color='#4488ff', zorder=5, label='SPARC Obs.')
for i, txt in enumerate(sparc_names):
    if txt in ['NGC 2841', 'Voie Lactée', 'M33', 'NGC 3741']:
        ax_S.annotate(txt, (sparc_masses[i], sparc_vobs[i]), xytext=(5, -10), 
                      textcoords='offset points', fontsize=9, color='white')

masses_sim_sol = np.logspace(8, 11.5, 100)
v_trame_sim_kms = ((G_SI * (masses_sim_sol * M_SUN) * a0)**0.25) / 1000
ax_S.plot(masses_sim_sol, v_trame_sim_kms, color='#ff8844', linestyle='-', label=r"Frame: $\sqrt[4]{G M a_0}$")
ax_S.legend(facecolor='#1a1a3a', labelcolor='white', loc='upper left')
ax_S.text(1e10, 45, r"$v_{Frame} = \sqrt[4]{G \cdot M_{bar} \cdot a_0}$", color='#ff8844', fontsize=14, bbox=eq_box)

pt_SS, = ax_S.plot([], [], 'o', color='yellow', ms=12, markeredgecolor='white', markeredgewidth=2, zorder=10, label="Current Sim")

# ── Sliders ──
ax_sM  = plt.axes([0.15, 0.15, 0.7, 0.025], facecolor='#1a1a3a')
ax_sV  = plt.axes([0.15, 0.10, 0.7, 0.025], facecolor='#1a1a3a')

# ON UTILISE LE LOGARITHME DE LA MASSE (de 10^8 à 10^11.5)
sl_M   = Slider(ax_sM, 'Log10(Mass M☉)', 8.0, 11.5, valinit=11.0,  color='#4488ff')
sl_V   = Slider(ax_sV, 'Orbit Speed',   10,   50,   valinit=30,    color='#88ff44', valstep=1)
...
def update_physics(val=None):
    ...
    # Update SPARC Marker
    v_asymp = ((G_SI * M_si * a0)**0.25) / 1000
    pt_SS.set_data([M_sol], [v_asymp])
    ...
    # ── Calcul Photons et Auto-Zoom ──
    state['ph_Nx'], state['ph_Ny'] = compute_photon(factor=1)
    state['ph_Tx'], state['ph_Ty'] = compute_photon(factor=2)
    
    # Trouver le point le plus bas atteint par le photon de la Trame
    min_y = np.min(state['ph_Ty'])
    delta_y = b - min_y
    if delta_y < 0.02: 
        delta_y = 0.02 # Eviter un zoom infini sur les très petites masses
        
    # On ajuste dynamiquement les limites de l'axe Y pour bien voir l'écart
    y_min_plot = b - delta_y * 1.5
    y_max_plot = b + delta_y * 0.2
    ax_P.set_ylim(y_min_plot, y_max_plot)
    
    # On place la "masse" jaune au bas de l'écran visible dynamiquement
    pt_mass_P.set_data([0], [y_min_plot - (y_max_plot - y_min_plot)*0.05])
    
    # Replacer dynamiquement le texte de la formule pour qu'il suive le zoom
    for txt in ax_P.texts:
        if "theta_{Frame}" in txt.get_text():
            txt.set_position((-76, y_max_plot - (y_max_plot - y_min_plot)*0.15))

    alpha_N = np.degrees(2*G*M/b)
    alpha_T = np.degrees(4*G*M/b)
    ln_NP.set_label(f'Newton  θ={alpha_N:.5f}°')
    ln_TP.set_label(f'Frame   θ={alpha_T:.5f}°')
...
def animate(frame):
    ...
    # Recalcul des vitesses actuelles pour le curseur dynamique
    M_sol = 10**(sl_M.val)
    M_si  = M_sol * M_SUN
    Rt_SI = np.sqrt(G_SI * M_si / a0)
    ...
    return ln_NO, ln_TO, pt_NO, pt_TO, ln_NP, ln_TP, pt_NP, pt_TP, pt_NG, pt_TG, ln_Rt, pt_SS
